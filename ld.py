# NPS 分数计算，TV Loss计算，patch对比度、亮度、噪声、旋转等变换
...
class MaxProbExtractor_yolov5(nn.Module):
    ...
    def forward(self, YOLOoutput):  # YOLOoutput为列表：[infer, loss_out]
        # print(len(YOLOoutput))  2
        # print(YOLOoutput[0].shape)  torch.Size([batch, 25200, 25])
        output = YOLOoutput[0].transpose(1, 2).contiguous()  # [batch, 25, 25200]
        output_objectness = torch.sigmoid(output[:, 4, :])  # [batch, 25200]
        output = output[:, 5:5 + self.num_cls, :]  # [batch, 20, 25200]
        # print(output.shape)
...
class NPSCalculator(nn.Module):
    ...
    def forward(self, adv_patch):
        ...
        color_dist = (adv_patch - self.printability_array + 0.000001)  # 计算补丁中的颜色与可打印性数组中的颜色之间的欧几里德距离
        color_dist = color_dist ** 2
        color_dist = torch.sum(color_dist, 1) + 0.000001
        color_dist = torch.sqrt(color_dist)
        # only work with the min distance
        color_dist_prod = torch.min(color_dist, 0)[0] #test: change prod for min (find distance to closest color) 每组中最小的距离是我们需要的
        # calculate the nps by summing over all pixels
        nps_score = torch.sum(color_dist_prod,0)
        nps_score = torch.sum(nps_score,0)
        return nps_score / torch.numel(adv_patch)

    def get_printability_array(self, printability_file, side):  # 读取可打印颜色表以生成和补丁像素值相减的矩阵
        printability_list = []
...
class PatchTransformer(nn.Module):
    ...
    def forward(self, adv_patch, lab_batch, img_size, do_rotate=True, rand_loc=True):
        ...
        adv_patch = self.medianpooler(adv_patch.unsqueeze(0))  # [1, 3, size0, size1]
        # Determine size of padding
        pad = (img_size - adv_patch.size(-1)) / 2
        # Make a batch of patches
        adv_patch = adv_patch.unsqueeze(0)#.unsqueeze(0)   [1, 1, 3, size0, size1]
        adv_batch = adv_patch.expand(lab_batch.size(0), lab_batch.size(1), -1, -1, -1)  # [batchsize, 50, 3, size0, size1]
        ...
        adv_batch = torch.clamp(adv_batch, 0.000001, 0.99999)  # [batchsize, 50, 3, size0, size1]

        # Where the label class_id is 1 we don't want a patch (padding) --> fill mask with zero's
        cls_ids = torch.narrow(lab_batch, 2, 0, 1)  # 取第3维的第一列(类别索引) 0~20 or -1  # [batchsize, lines, 5] => [batchsize, lines, 1]
        cls_ids = cls_ids.view(lab_batch.size(0), -1).cpu()
        cls_ids[:, 1::2] = 1  # 一半目标不apply patch
        
        cls_ids = cls_ids.unsqueeze(-1).cuda()
        cls_mask = cls_ids.expand(-1, -1, 3)  # [batchsize, lines, 3]
        cls_mask = cls_mask.unsqueeze(-1)  # [batchsize, lines, 3, 1]
        cls_mask = cls_mask.expand(-1, -1, -1, adv_batch.size(3))  # [batchsize, lines, 3, size0]
        cls_mask = cls_mask.unsqueeze(-1)  # [batchsize, lines, 3, size0, 1]
        cls_mask = cls_mask.expand(-1, -1, -1, -1, adv_batch.size(4))  # [batchsize, lines, 3, size0, size1]
        # msk_batch = torch.cuda.FloatTensor(cls_mask.size()).fill_(1)
        msk_batch = torch.cuda.FloatTensor(cls_mask.size()).fill_(1) - cls_mask  # mask   =1: patch    =0: no patch
        ...
        lab_batch_scaled[:, :, 1] = lab_batch[:, :, 1] * img_size  # 中心坐标
        lab_batch_scaled[:, :, 2] = lab_batch[:, :, 2] * img_size
        lab_batch_scaled[:, :, 3] = lab_batch[:, :, 3] * img_size  # 长宽
        ...
        if(rand_loc):  # patch中心点偏移, 一般情况下: rand_loc = False
            off_x = targetoff_x * (torch.cuda.FloatTensor(targetoff_x.size()).uniform_(-0.5, 0.5))
            target_x = target_x + off_x
            off_y = targetoff_y * (torch.cuda.FloatTensor(targetoff_y.size()).uniform_(-0.5, 0.5))
            target_y = target_y + off_y
            
        # Put the patch in the upper of the target
        # target_y = target_y - 0.12 * (target_size.view(np.prod(batch_size)) / current_patch_size / 3 * 5)
        ...
        scale = target_size / current_patch_size  # 缩放比例
        ...
        theta[:, 0, 0] = cos / scale  # 前两列为旋转矩阵
        theta[:, 0, 1] = sin / scale
        theta[:, 0, 2] = tx * cos / scale + ty * sin / scale  # 第三列为平移比例, 向右和向下为负
        theta[:, 1, 0] = -sin / scale
        theta[:, 1, 1] = cos / scale
        theta[:, 1, 2] = -tx * sin / scale + ty * cos / scale

        b_sh = adv_batch.shape
        grid = F.affine_grid(theta, adv_batch.shape)  # 创建grid, adv_batch.shape仿射变换后得到图像大小

        adv_batch_t = F.grid_sample(adv_batch, grid)  # 重采样
...
class PatchApplier(nn.Module):
    ...
    def forward(self, img_batch, adv_batch):
        advs = torch.unbind(adv_batch, 1)  # 张量切片
        for adv in advs:
...
class InriaDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        image = transform(image) # 包含除以255操作 
    ...
    def pad_lab(self, lab):
        ...
        if(pad_size > 0):
            # padded_lab = F.pad(lab, (0, 0, 0, pad_size), value=-1)  # 填充值为-1
            padded_lab = F.pad(lab, (0, 0, 0, pad_size), value=1)  # 填充值为1
...
if __name__ == '__main__':
    ...
    prob_extractor = MaxProbExtractor_yolov5(0, 80).cuda()
